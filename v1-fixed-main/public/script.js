...
let pollTimer     = null;
const POLL_MS     = 30000; // poll every 30s — reduces flicker
...
function renderContainers() {
  ...
  // Build map of existing cards so we UPDATE them instead of wiping the DOM
  // This stops the visual "flash/reload" on every poll
  const existingCards = {};
  grid.querySelectorAll('.container-card[data-id]').forEach(el => {
    existingCards[el.dataset.id] = el;
  });

  // If container count changed, do a full rebuild (first load or containers added)
  ...
  containers.forEach((c, i) => {
    ...
    if (!needsRebuild && existingCards[c.id]) {
      // ✅ SMART UPDATE — only patch changed values, no DOM wipe = no flicker
      const card = existingCards[c.id];
      card.querySelector('.status-badge').className   = `status-badge ${c.status}`;
      card.querySelector('.status-badge').textContent = c.status;
      card.querySelector('.card-progress-label span:first-child').textContent = `${pct}% filled`;
      card.querySelector('.card-progress-label span:last-child').textContent  = `${c.quantity}/${c.capacity}`;
      const bar = card.querySelector('.card-progress-bar');
      bar.style.width = `${pct}%`;
      bar.className   = `card-progress-bar ${barClass}`;
      card.querySelector('.card-qty').textContent = c.quantity;
      card.querySelector('.card-cap').textContent = `cap. ${c.capacity}`;
      card.querySelector('.card-log-count').innerHTML = `
    ...
    // First-time build for this card
